...
def run_comparison():
    ...
    def get_param_snapshot(model):
        # Track first two weights of the first conv layer
        # model.conv1 is QuantizedConv2d, model.conv1.conv is nn.Conv2d
        w = model.conv1.conv.weight.data
        return (w[0, 0, 0, 0].item(), w[0, 0, 0, 1].item())
    ...
    for e in range(1, epochs + 1):
        ...
        for d, t in train_loader:
            ...
            if len(ep_loss) % 20 == 0:
                traj_prox.append(get_param_snapshot(model_prox))
    ...
    for e in range(1, epochs + 1):
        ...
        for d, t in train_loader:
            ...
            ep_loss.append(loss.item())
            if len(ep_loss) % 20 == 0:
                traj_adamw.append(get_param_snapshot(model_adamw))
    ...
    for e in range(1, epochs + 1):
        ...
        for batch_idx, (d, t) in enumerate(train_loader):
            ...
            if batch_idx % 20 == 0:
                traj_sadam.append(get_param_snapshot(model_sadam))
    ...
    return losses_prox, losses_adamw, losses_sadam, opt_sadam.lgi_history, traj_prox, traj_adamw, traj_sadam
...
if __name__ == "__main__":
    ...
    l_prox, l_adamw, l_sadam, lgi, t_prox, t_adamw, t_sadam = run_comparison()
    ...
    print("Saved plot to sadam_vs_proxsgd_adamw_CIFAR100.png")
    ...
    plt.title("Optimization Trajectory on Standard Rosenbrock")
    plt.xlabel("x")
    plt.ylabel("y")
    plt.legend()
    ...
    plt.ylim(-1.0, 4.0)
    plt.tight_layout()
    plt.savefig('chattering_trajectory_CIFAR100.png', dpi=300)
    print("Saved chattering plot to chattering_trajectory_CIFAR100.png")
