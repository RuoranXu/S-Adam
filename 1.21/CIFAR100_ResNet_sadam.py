    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p in params_with_grad:
                ...
                if group['l1_lambda'] > 0:
                    threshold = group['lr'] * group['l1_lambda']
...
# ==============================================================================
# Part 3: Model Definition (ResNet18 Pretrained)
# ==============================================================================
def get_pretrained_model(num_classes=10):
    ...
    # Freeze earlier layers to save compute/memory (optional, but good for transfer learning)
...
def run_comparison():
    # Setup Data - Using CIFAR-10 instead of TinyImageNet
    # Resize to 224x224 to match ResNet input expectations (ImageNet standard)
    ...
    print("Loading CIFAR-100 (as proxy for ImageNet)...")
    ...
    # Smaller batch size due to larger image size (224x224)
    ...
    epochs = 6  # Reduced epochs for demonstration
    ...
    model_prox = get_pretrained_model(num_classes=100).to(device)
    ...
    # Record max memory
    memory_prox = torch.cuda.max_memory_allocated() / (1024 ** 2) if torch.cuda.is_available() else 0
    
    # Cleanup
    del model_prox, opt_prox
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    ...
    torch.manual_seed(42)
    model_adamw = get_pretrained_model(num_classes=100).to(device)
    ...
    # Record max memory
    memory_adamw = torch.cuda.max_memory_allocated() / (1024 ** 2) if torch.cuda.is_available() else 0
    
    # Cleanup
    del model_adamw, opt_adamw
    if torch.cuda.is_available():
        torch.cuda.empty_cache()
    ...
    torch.manual_seed(42)
    model_sadam = get_pretrained_model(num_classes=100).to(device)

    opt_sadam = SAdam(model_sadam.parameters(),
                      lr=0.001,  # Lower LR for fine-tuning
                      k_directions=8, # Reduced k for speed
                      sigma=0.01,
                      lgi_lambda=2.0,
                      l1_lambda=1e-4)
    ...
    # Record max memory
    memory_sadam = torch.cuda.max_memory_allocated() / (1024 ** 2) if torch.cuda.is_available() else 0

    # Capture LGI history before cleanup
    lgi_history_save = opt_sadam.lgi_history
    ...
    return losses_prox, losses_adamw, losses_sadam, lgi_history_save
...
if __name__ == "__main__":
    sys.stdout = Logger('training_log_CIFAR100_resnet_learning1.txt')
    ...
    ax1.plot(l_sadam_smooth, color='blue', linewidth=2, label='S-Adam (Ours)')
    ...
    plt.title("S-Adam vs Prox-SGD vs AdamW (Transfer Learning ResNet18)")
    plt.tight_layout()
    plt.savefig('sadam_vs_proxsgd_adamw_CIFAR100_resnet1.png', dpi=300)
    print("Saved plot to sadam_vs_proxsgd_adamw_CIFAR100_resnet1.png")
    plt.show()
