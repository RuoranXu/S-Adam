...
def run_comparison():
    ...
    opt_prox = ProxSGD(model_prox.parameters(), lr=0.002, momentum=0.9, l1_lambda=1e-4)
    ...
    print("="*40)
    print(f"{'Method':<20} | {'Total Time (s)':<15}")
    print("-" * 40)
    print(f"{'Prox-SGD (Base)':<20} | {total_time_prox:.4f}")
    print(f"{'S-Adam (Ours)':<20} | {total_time_sadam:.4f}")
    print("-" * 40)
    
    # Calculate Fairness Factor
    ratio = total_time_sadam / total_time_prox
    print(f"Cost Factor: S-Adam is {ratio:.2f}x slower than Baseline.")
    print("="*40 + "\n")

    return losses_prox, losses_sadam, opt_sadam.lgi_history

# ==============================================================================
# Visualization
# ==============================================================================
if __name__ == "__main__":
    sys.stdout = Logger('training_log_prox_vs_sadam_v3.txt')
    
    l_prox, l_sadam, lgi = run_comparison()
    ...
    plt.title("S-Adam vs Prox-SGD (Final Tuning)")
    plt.tight_layout()
    plt.savefig('sadam_vs_proxsgd_v3.png', dpi=300)
    print("Saved plot to sadam_vs_proxsgd_v3.png")
    plt.show()